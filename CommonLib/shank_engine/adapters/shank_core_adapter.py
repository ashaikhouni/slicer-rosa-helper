...
def run_blob_ransac_v1(ctx: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    raw = run_detection(
        ...
        # V1 is now intentionally voxel-only. Blob support lives in blob_em_v2.
        ...
        include_debug_masks=bool(config.get("debug_masks", False)),
    )
    ...
    diagnostics = {
        ...
        "effective_min_inliers": int(config.get("min_inliers", 250)),
        "effective_inlier_radius_mm": float(config.get("inlier_radius_mm", 1.2)),
