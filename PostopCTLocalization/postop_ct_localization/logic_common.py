...
class PostopCTLocalizationLogicBaseMixin:
    ...
    def sync_manual_trajectories_to_workflow(
        ...
        nodes = []
        n_reoriented = 0
        # Electrode-model picking is deferred to Contacts & Trajectory View's
        # "Suggest models" / "Generate contacts" action, which invokes the
        # canonical matched-filter picker in
        # ``rosa_core.contact_placement.stage_d_pick``. The Manual Fit-time
        # PaCER classifier was removed 2026-05-11 — CTV defaults its
        # dropdowns to empty regardless of ``Rosa.BestModelId``.
        for row in rows:
