...
class GuidedFitWidgetMixin:
    ...
    def _fit_seeds(self, seed_rows):
        ...
        # Guided Fit snaps the user's seeds directly via the canonical
        # snap-flow (run_seeded_fit) — no auto-detection / match-against-auto
        # detour. The provided seed IS the candidate; the batch snap arbitrates
        # cross-shank conflicts between the seeds.
        ...
        # Batch-snap all seeds in one run_seeded_fit call (cross-shank
        # arbitration between them) — the shared engine the CLI uses too.
        ...
        for rec in fit_records:
            ...
            self.log(
                "[guided] {n}: snap  angle={a:.2f}°  lat={l:.2f} mm  "
                "n={ni}  len={L:.1f} mm".format(
                    n=name,
